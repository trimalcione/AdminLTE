...
if (isset($_GET['getAllQueries']) && $auth)
{
	...
	if($_GET['getAllQueries'] !== "empty")
	{
		...
		$dbquery = "SELECT timestamp, type, domain, client, status, additional_info FROM queries WHERE timestamp >= :from AND timestamp <= :until ";
		if(isset($_GET["types"]))
		{
			$types = $_GET["types"];
		...
		if(!is_bool($results))
			while ($row = $results->fetchArray())
			{
				...
				// array:        time     type         domain                client           status   additional_info
